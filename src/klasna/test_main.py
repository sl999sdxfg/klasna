...
def test_student_parent_unlink(created_student, created_parent):
    student_id = created_student["id"]
    parent_id = created_parent["id"]
    print(created_parent)
    post_response = client.post(f"{baseurl}/students/{student_id}/parents/{parent_id}")
    ...
    delete_response = client.delete(
        f"{baseurl}/students/{student_id}/parents/{999_999_999}"
    )
    assert delete_response.status_code == status.HTTP_404_NOT_FOUND

    delete_response = client.delete(
        f"{baseurl}/students/{999_999_999}/parents/{parent_id}"
    )
    assert delete_response.status_code == status.HTTP_404_NOT_FOUND
